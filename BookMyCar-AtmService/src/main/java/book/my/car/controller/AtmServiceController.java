...
public class AtmServiceController {
	...
	public ResponseEntity<List<CarBookingResponse>>  getBookingDetails(@PathVariable String email){
		System.out.println(email);
		
		List<CarBookingResponse> bookingDetailsByEmail = atmService.getBookingDetailsByEmail(email);
		
		return ResponseEntity.status(HttpStatus.CREATED).body(bookingDetailsByEmail);
	}
}
